...
if __name__ == '__main__':
    ...
    from Backbone import InvertedResidual
    from config import ActivityConfig as cfg
    from tsm_plugin import ConsensusModule, Identity
...
class TSM(nn.Module):
    def __init__(self,
        ...
        feature_dim = self._prepare_tsm(num_class)
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            self.base_model = getattr(Backbone, 'mobilenet_v2')(pretrained=self.pretrain)
            ...
            self.base_model = getattr(Backbone, 'bninceptionTSM')(pretrained=self.pretrain)
            self.input_size = 224
...
def test_inference_time():
    ...
    seg = 32
    print("=> Testing segments %d"%(seg))
    x = torch.ones([1, seg*3, 224, 224])
    print(x.shape)
    x = x.cuda()
    net = TSM(51,
              seg,
    ...
    for i in range(_len):
        ...
        if i<50:
            ...
            avg.append((t1 - t0))
            print("[%d]time is %.6f" % (i, (t1 - t0)))
    avg_t = sum(avg) /len(avg)
    print("=> len:%d"%(len(avg)))
    print("=> avg time is %.6f" % (avg_t))
...
if __name__ == '__main__':
    ...
    net = TSM(cfg.DATASET.CLASS_NUM,
            ...
            print_spec=True,
            pretrain=cfg.PRETRAIN_TYPE,
            is_shift=cfg.TRAIN.IS_SHIFT,
            shift_div=cfg.TRAIN.SHIFT_DIV,
            shift_place= 'block' if cfg.MODEL_NAME == 'tsm' and cfg.BACKBONE == 'BNInception' else 'blockres',
            fc_lr5=False,
            temporal_pool=False,
            non_local=False)
