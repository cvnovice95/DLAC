...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,zero_init_residual=False,
        ...
        #TODO: Local Branch
        self.up2d = nn.UpsamplingBilinear2d(size=(14,14))
        #TODO: Global Branch
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        #TODO: Discriminative Branch
        self.fusion_conv = nn.Conv2d(512 * (block.expansion+2),num_classes*self.DFB_num,kernel_size=1,stride=1,padding=0,bias=False)
        self.fusion_maxpool =  nn.AdaptiveMaxPool2d((1, 1))
        self.fusion_fc = nn.Linear(num_classes*self.DFB_num,num_classes)
    ...
    def _forward(self, x):
        ...
        z = self.fusion_maxpool(z)
        z_xchannel = torch.mean(z.view(-1,self.DFB_num,self.num_classes,1),dim=1).view(-1,self.num_classes)
        z_max = self.fusion_fc(torch.flatten(z.view(-1,self.DFB_num*self.num_classes),1))
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        for layer_name, param in pretrain_dict.items():
...
if __name__ == '__main__':
    ...
    net = resnet50(pretrained=True,progress=True,num_classes=51).cuda()
    z_avg,z_max,z_channel = net(x)
    print(z_avg.shape)
    print(z_max.shape)
