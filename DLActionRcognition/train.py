...
if cfg.REDIS_MODE:
    from data_loader import train_dpflow_dataloader_dp, val_dpflow_dataloader_dp
...
def main():
    ...
    if not cfg.REDIS_MODE:
        if cfg.USE_DISTRIBUTED:
            t_loader,t_sampler = train_dataloader(train_transform)
            v_loader = val_dataloader(val_transform)
        else:
            t_loader = train_dataloader(train_transform)
            v_loader = val_dataloader(val_transform)
    else:
        t_loader = train_dpflow_dataloader_dp(train_transform)
        v_loader = val_dpflow_dataloader_dp(val_transform)
    ...
    if torch.cuda.is_available():
        if torch.cuda.device_count() > 1:
            if cfg.USE_DISTRIBUTED:
                    ...
                    net = DistributedDataParallel(net, device_ids=[cfg.local_rank], output_device=cfg.local_rank)
            else:
