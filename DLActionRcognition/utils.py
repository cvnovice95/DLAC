...
Description: It's used to count variable and calculate average value
'''
...
Description: It's used to record model's evaluation info.
'''
...
'''
Description: Some function which is used in distributed training
'''
def synchronize():
    """
...
class PipeOutput(object):
    def __init__(self,name=None):
        ...
        try:
            self.handle_sender = os.open(self.out_path, os.O_SYNC | os.O_CREAT | os.O_RDWR)
        except Exception:
            exc_type, exc_value, exc_traceback_obj = sys.exc_info()
            traceback.print_tb(exc_traceback_obj)
    ...
    def put(self,data):
        ...
        os.write(self.handle_sender,p_data)
        # time.sleep(1)

    def close(self):
        os.close(self.handle_sender)
