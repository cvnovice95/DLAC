...
def bninceptionTSM(pretrained='imagenet'):
    ...
    if pretrained is not None:
        if pretrained == 'imagenet':
            ...
            model = BNInception(num_classes=400)
            ## TODO: load pretrain model
            if not os.path.exists(cfg.PRETRAIN_MODEL_ZOO):
                print("=> %s don't exist,will be created!!!"%(cfg.PRETRAIN_MODEL_ZOO))
                os.makedirs(cfg.PRETRAIN_MODEL_ZOO)
            ...
            print("=> pretrain type %s don't support it"%(pretrained))
            sys.exit(0)
    else:
        model = BNInception(num_classes=1000)
